  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-8 md:px-8">
      <header className="mb-8 flex items-end justify-between">
        <div>
          <p className="text-muted-foreground font-mono text-xs uppercase tracking-widest">Welcome back</p>
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        ...
        <Button asChild size="sm" variant="outline">
          <Link href="/courses"><BookOpen className="size-4" /> Courses</Link>
        </Button>
        <Button asChild size="sm" variant="outline">
          <Link href="/chat"><MessageSquare className="size-4" /> Chat</Link>
        </Button>
      </div>

      {courses.length > 0 && (
        <div className="mb-6">
          <h3 className="text-muted-foreground mb-3 text-xs font-medium uppercase tracking-wider">Jump to course</h3>
          <div className="grid gap-2 sm:grid-cols-2">
            {courses.map((c) => (
              <div key={c.id} className="bg-card flex items-center gap-3 rounded-xl border px-4 py-3">
                <span className="size-3 shrink-0 rounded-full" style={{ backgroundColor: c.color ?? 'var(--primary)' }} />
                ...
                <div className="flex items-center gap-1">
                  ...
                  <Button asChild variant="ghost" size="sm" className="h-7 w-7 px-0">
                    <Link href={`/courses/${c.id}/quizzes`}><FlaskConical className="size-3.5" /></Link>
                  </Button>
                </div>
              </div>
            ))}
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            ...
            <div className="mt-3 flex items-center gap-1.5">
              ...
              {['bg-muted/60','bg-primary/30','bg-primary/60','bg-primary/80','bg-primary'].map((c) => (
      ...
      {courses.length === 0 && (
        <Card className="border-dashed">
          ...
          <CardFooter>
            <Button asChild>
              <Link href="/courses">Go to Courses <ArrowRight className="size-4" /></Link>
            </Button>
          </CardFooter>
        </Card>
      )}
